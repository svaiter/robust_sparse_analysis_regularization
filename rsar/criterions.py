...
def _ic_minimization(DJ, Omega_x0):
    ...
    w = douglas_rachford(prox_indic, prox_obj, np.zeros(np.size(DJ,1)))
    return np.max(np.abs(Omega_x0 - w))

def criterions_dict(D, Phi, x):
    ...
    U = null(DJ.T, row_wise_storage=False)
    gram = np.dot(Phi.T, Phi)
    A = np.dot(np.dot(U, scipy.linalg.pinv(np.dot(np.dot(U.T, gram), U))),
        U.T)
    ...
    if np.prod(null(DJ).shape) == 0:
        ic_ker = lambda s: _ic_minimization(DJ, np.dot(Omega, s))
    else:
        ic_ker = ic_noker
